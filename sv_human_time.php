...
class sv_human_time extends init {
	...
	public function init() {
		...
		$this->set_module_desc( __( 'This module converts dates and times to a human readable text, via the "[sv_human_time]" shortcode.', 'straightvisions_100' ) );
	...
	protected function load_settings(): sv_human_time {
		$this->s['posts'] =
			static::$settings
				->create( $this )
				->set_ID( 'posts' )
				->set_title( __( 'Enables relative date format for all posts', 'straightvisions_100' ) )
				->set_default_value( 1 )
				->load_type( 'checkbox' );
		...
		$this->s['comments'] =
			static::$settings
				->create( $this )
				->set_ID( 'comments' )
				->set_title( __( 'Enables relative date format for all comments', 'straightvisions_100' ) )
				->set_default_value( 1 )
				->load_type( 'checkbox' );
		...
		$this->s['date_after'] =
			static::$settings
				->create( $this )
				->set_ID( 'date_after' )
				->set_title( __( 'Show Date Format', 'straightvisions_100' ) )
				->set_description( __( 'Shows the date and time in the default WordPress format, when the time difference is higher than the set days.<br>0 = never', 'straightvisions_100' ) )
				->set_default_value( 0 )
				->set_min( 0 )
				->load_type( 'number' );
	...
	protected function router( array $settings ): string {
		if ( $settings['date_start'] ) {
			...
			return __( 'Starting date is needed!', 'straightvisions_100' );
		}
		...
		if ( $date_after > 0 && $time_diff > $date_after ) {
			...
			$date = sprintf( __('%s ago', 'straightvisions_100' ), $date );
		}
